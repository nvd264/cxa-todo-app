  ...
  render() {
    return (
      <ScrollView style={styles.container}>
        <Container>
          <SearchBar placeholder="Search Completed" onChangeSearch={this.onChangeSearch} />
          <Content>
            {this.props.todos.filter(todo => todo.completed && todo.title.indexOf(this.state.search) !== -1).map((todo, idx) =>
                <ToDo
                  key={idx}
                  todo={todo}
